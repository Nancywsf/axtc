    ...
    <div class="content-tabContent">
      ...
      <div class="_zx">
        <ul class="main-content">
          <li v-for="item in zxOrderList">
            <div class="item-top">
              <div class="fl order-data title">订单日期：{{formatDate(item.add_time)}}</div>
            ...
            <div class="item-bottom">
              ...
              <!--<router-link v-if="item.status === 1" :to="{path:'/user/order/orderInfo', query:{oid:item.id}}" class="btn-openOrder fr" :data-id="item.id">查看</router-link>-->
        ...
        <p class="no-result" v-if="zxOrderList.length == 0">您还没有装修订单，赶紧去预约吧 (●'◡'●)ﾉ♥ </p>
        <v-pagination v-if="zxOrderList.length > 0" :page-data="zxPageData" v-on:goPage="getZXOrderList"></v-pagination>
      </div>
      <!--装修订单 结束-->
    </div>
  </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      this.getZXOrderList(this.zxPageData)
    },
    methods: {
      // 获取装修订单
      getZXOrderList: function (data) {
        let params = {
          page: data.page - 1,
        ...
        axios.post(this.$store.state.HOST + '/zxpc/my/myorder', params,
          ...
          this.zxOrderList = response.data
          this.zxPageData.pageNo = response.pageCount
        })
      },
      // 订单状态 数字 转 文字
